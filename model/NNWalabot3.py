...
# DNN-10240x4096 DNN-4096x4096 DNN-4096x1024 (3)
...
class modelX():
  def __init__(self):
    ...
    # Input & label
    self.x = tf.placeholder(tf.float32,[None,1,2048,40])
    ...
    self.half_bnm1 = beta([40])
    self.half_bnv1 = beta([40])
    ...
    self.half_bnm2 = beta([40])
    self.half_bnv2 = beta([40])
    ...
    self.half_bnm3 = beta([40])
    self.half_bnv3 = beta([40])
    ...
    # FC-1
    self.w1 = weight([10240,4096])
    self.b1 = bias([4096])
    self.bng5 = gamma([4096])
    self.bnb5 = beta([4096])
    self.bnm5 = beta([4096])
    self.bnv5 = beta([4096])

    # FC-2
    self.w2 = weight([4096,2048])
    self.b2 = bias([2048])
    ...
    self.cnn4_output = tf.reshape(self.cnn4,[-1,256*40])
  ...
  def loss(self,X,y,X1,y1,e,mode='test',lr=2e-4,reg=1e-5,batch=5,epoch=21,opt=True,verbose=True):
    ...
    cross_entropy = tf.add(cross_entropy, tf.mul(_reg,self.regularizers))
...
def test():
  ...
  y = np.random.randint(2,size=(10,1024)).astype(np.int32)
